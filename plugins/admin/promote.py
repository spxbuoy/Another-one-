    ...
    try:
        ...
        if message.reply_to_message:
            ...
            try:
                ...
                await message.reply_text("⚠️ <b>Usage:</b> <code>/pm user_id</code> or reply to a user", quote=True, parse_mode="html")
                return

        # Get user info
        user_data = fetchinfo(target_id)
        if not user_data:
            await message.reply_text("❌ <b>User not found in database.</b>", quote=True, parse_mode="html")
            return
